...
def open_playing_window(game, window, bg, front_images):
    ...
    display_init_fronts(game=game, can=can, playing_window=playing_window, rows=rows, columns=columns, line_height=line_height,
                        column_width=column_width, front_images=front_images, back_image=back_image, countdown_label=countdown_label, attempts_label=attempts_label)


# list est la liste des images en format Image
def display_init_fronts(game, can: Canvas, playing_window, rows, columns, line_height, column_width, front_images, back_image, countdown_label, attempts_label):
    images_id = []
    for l in front_images:
        images_id.append(['']*len(l))
    for i in range(rows):
        for j in range(columns):
            images_id[i][j] = can.create_image(
                j*column_width + column_width/2, i*line_height + line_height/2, image=front_images[i][j])
    ...
    can.bind("<Button-1>", lambda event: on_click(game, event, can, images_id, front_images, line_height, column_width,
...
def special4(game, can, playing_window, images_id, front_images, back_image):
            ...
            if not card.flipped and card.id not in game.flipped:
                ...
                if (card.power == 0):
                    ...
                    if card.power == 1:
                        ...
                        game.flipped.append(card.id)
                        special4(game, can, playing_window, images_id, get_front_images(
                            game), back_image)
